...
{
   int distance = 1000;
   if (abs(static_cast<int>(first.PosA) - static_cast<int>(first.PosA1)) > distance) {
      return false;
   }
   if (abs(static_cast<int>(first.PosB) - static_cast<int>(first.PosB1)) > distance) {
      return false;
   }
   if (abs(static_cast<int>(second.PosA) - static_cast<int>(second.PosA1)) > distance) {
      return false;
   }
   if (abs(static_cast<int>(second.PosB) - static_cast<int>(second.PosB1)) > distance) {
      return false;
   }
   float cutoff = 0.9;
...
void ModifyRP(std::vector <RP_READ> & Reads_RP)
{
   ...
   for (unsigned first = 0; first < Reads_RP.size(); first++) {
      ...
      if (Reads_RP[first].ChrNameA == Reads_RP[first].ChrNameB && abs(static_cast<int>(Reads_RP[first].PosA) - static_cast<int>(Reads_RP[first].PosB)) < 500) {
            ...
            if ( firstChrName!="" && secondChrName!="" ) {
               ...
               #pragma omp critical
               {
                  ...
                  RPoutputfile << firstChrName  << "\t" << ( (firstPos > g_SpacerBeforeAfter) ? firstPos - g_SpacerBeforeAfter : 1)
                               ...
                               << abs(static_cast<int>(secondPos) - static_cast<int>(firstPos)) << "\tSupport: " << currentState.Reads_RP_Discovery[read_index].NumberOfIdentical << "\t";
